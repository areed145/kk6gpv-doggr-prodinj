...
def prodinj(api):
    ...
    try:
        ...
        inj_ = False
    try:
        inj.loc[inj['welltype_i'].isin(['WF','WD']), 'water_i'] = inj['wtrstm']
        inj.loc[inj['welltype_i'] == 'SF', 'steam'] = inj['wtrstm']
        inj.loc[inj['welltype_i'] == 'SC', 'cyclic'] = inj['wtrstm']
        inj.loc[inj['welltype_i'].isin(['GD','GS','PM']), 'gas_i'] = inj['wtrstm']
    ...
    prodinj = pd.DataFrame(columns = ['date', 'oil', 'water', 'gas', 'daysprod', 'oilgrav', 'pcsg', 'ptbg',
        ...
        'poolcode_p', 'wtrstm', 'water_i', 'steam', 'cyclic', 'gasair', 'gas_i', 'air', 'daysinj', 'pinjsurf', 'wtrsrc', 'wtrknd',
    ...
    for col in ['oil', 'water', 'gas', 'daysprod', 'wtrstm', 'water_i', 'steam', 'cyclic', 'gasair', 'gas_i', 'air', 'daysinj']:
    ...
    prodinj.loc[(prodinj['cyclic_last'] > 0) & (prodinj['oil_last'] == 0), 'cyclic_ct'] = 0
    ...
    db.doggr.update_one({'api': api}, {'$set': {'prodinj': data}}, upsert=False)

    try:
        db.doggr.update_one({'api': api}, {'$unset': {'cyclic_jobs':1}}, upsert=False)
    except:
        pass

    try:
        db.doggr.update_one({'api': api}, {'$unset': {'prod':1}}, upsert=False)
    except:
        pass
    ...
    try:
        ...
        data = json_util.loads(json.dumps(dict_header['cyclic_jobs'], cls=NpEncoder))
        db.doggr.update_one({'api': api}, {'$set': {'cyclic_jobs': data}}, upsert=False)
    except:
        pass
